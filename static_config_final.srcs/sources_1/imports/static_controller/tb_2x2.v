...
module tb_2x2;
    ...
    initial begin
        ...
        // allocate 2 vns for 2 rows of A 
        // vn1 : MS0,MS1
        // vn2 : MS2,MS3

        // then we will multicast columns of B with psum=2
        ...
        // art vn config
        rn_config_data = {3'd0,3'b011,3'd0,3'd0,3'd0,6'b100010};
        rn_config_en = 1;
        @(posedge CLK);
        rn_config_en = 0;
        @(posedge CLK);
        ...
        // stationary config
        mn_config_data = {80'd0,STATE_FILL_STATIONARY,16'd0,STATE_FILL_STATIONARY,16'd0,STATE_FILL_STATIONARY,16'd0,STATE_FILL_STATIONARY,16'd0};
        ...
        input_data = 16'd1;
        dn_config_data = 8'b00000001;// MS0
        @(posedge CLK);
        dn_config_en = 1;
        input_en = 1;
        @(posedge CLK);
        input_data = 16'd2;
        dn_config_data = 8'b00000010; // MS1
        @(posedge CLK);
        input_data = 16'd3;
        dn_config_data = 8'b00000100; // MS2
        @(posedge CLK);
        input_data = 16'd4;
        dn_config_data = 8'b00001000; // MS3
        @(posedge CLK);
        input_en = 0;
        dn_config_en = 0;
        @(posedge CLK);
        ...
        mn_config_data = {80'd0,STATE_MULT_STREAMING,16'd2,STATE_MULT_STREAMING,16'd2,STATE_MULT_STREAMING,16'd2,STATE_MULT_STREAMING,16'd2};
        mn_config_numSwitches = 4;
        mn_config_en = 1;
        @(posedge CLK);
        mn_config_en = 0;
        ...
        // multicast data
        input_data = 16'd5;
        dn_config_data = 8'b00000101; // MS0 and MS2
        @(posedge CLK);
        dn_config_en = 1;
        input_en = 1;
        @(posedge CLK);
        input_data = 16'd6;
        dn_config_data = 8'b00001010; // MS1 and MS3
        @(posedge CLK);
        input_data = 16'd7;
        dn_config_data = 8'b00000101; // MS0 and MS2
        @(posedge CLK);
    ...
    // Dump Waves (optional but helpful)
    initial begin
        $dumpfile("tb_2x2.vcd");
        $dumpvars(0, tb_2x2);
    end
